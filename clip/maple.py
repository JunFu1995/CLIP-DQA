            ...
            if len(compound_prompts_deeper) > 0:
                ...
                if not self.text_layer:
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        ...
                        visual_context = visual_context.expand(x.shape[1], -1, -1).permute(1, 0, 2).half() # L,B,C
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        ...
                        textual_context = textual_context.expand(x.shape[1], -1, -1).permute(1, 0, 2).half() # L, n_cls, ctx_dim
                        # Add the learnable tokens of this layer with the input, replaced by previous
                        # layer learnable tokens
                        x = torch.cat([prefix, textual_context, suffix], dim=0)
                        # Once done, update the counter, so that the next time, it does not use same learnable tokens
...
class VisionTransformer(nn.Module):
    ...
    def forward_web(self, x: torch.Tensor):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat(
...
class VisionTransformer_MaPLe(nn.Module):
    ...
    def tokenize(self, x: torch.Tensor):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  + self.positional_embedding.to(x.dtype)[1:]  # shape = [*, grid ** 2, width]
        return x 
    
    def forward_merge(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts):

        #add CLS token
        x = torch.cat(
            [self.class_embedding.to(x.dtype) + self.positional_embedding.to(x.dtype)[:1] + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device),
             x], dim=1)  # shape = [*, grid ** 2 + 1, width]
  
        # After positional embeddings, we will attach prompts with the model, remember only those
        # are trainable parameters here in whole image encoder.
        if len(shared_ctx): # self.VPT_shallow:
            if len(shared_ctx.shape) > 2:
                visual_ctx = shared_ctx.half().permute(1,0,2) # LBC -> BLC
            else:
                visual_ctx = shared_ctx.expand(x.shape[0], -1, -1).half()
            # print(x.shape, visual_ctx.shape)
            x = torch.cat([x, visual_ctx], dim=1)
        else:
            assert self.prompt_till_layer_visual == 0
        ...
        x = self.ln_post(x[:, 0, :]) # here cls -> all 
